...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {  guard let _ = (scene as? UIWindowScene) else { return }
        ...
        let firstNavController = UINavigationController(rootViewController: habitsVC)
        let secondNavController = UINavigationController(rootViewController: infoVC)
        
        let tbController = UITabBarController()
        tbController.tabBar.tintColor = UIColor(named: "purpuleColor")
        tbController.tabBar.backgroundColor = UIColor(named: "lightGrayColor")
